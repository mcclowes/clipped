...
struct LinkMetadataFetcherTests {
    ...
    func parseTitle() async throws {
        ...
        let metadata = await fetcher.fetchMetadata(for: #require(URL(string: "https://example.com")))
    ...
    func nonHttpUrl() async throws {
        let fetcher = LinkMetadataFetcher()
        let metadata = await fetcher.fetchMetadata(for: #require(URL(string: "ftp://example.com")))
        #expect(metadata.title == nil)
